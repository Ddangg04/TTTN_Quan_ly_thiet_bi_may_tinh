...
nv_add_hook($module_name, 'get_module_admin_theme', $priority, function ($vars) {
    ...
    if (in_array($module_name, ['upload', 'themes', 'emailtemplates', 'settings', 'seotools', 'modules', 'extensions', 'webtools', 'language', 'siteinfo', 'authors', 'database', 'comment', 'myapi','devices'])) {
        return $new_theme;
    }
    ...
    if (($module_info['module_file'] ?? '') == 'feeds' and in_array($op, ['main'])) {
      ...
      if (($module_info['module_file'] ?? '') == 'devices' and in_array($op, ['main', 'detail', 'category'])) {
        return $new_theme;
    }
